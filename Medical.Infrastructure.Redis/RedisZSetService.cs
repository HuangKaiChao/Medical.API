...
namespace Medical.Infrastructure.Redis
{
    [Provider_, Inject_]
    public class RedisZSetService:RedisBase
    {
        ...
        #region 获取
        // 获取带分数的有序集合成员
        public Dictionary<string, double> GetRangeWithScores(string key, int start, int stop, bool descending = true)
        {
            var results = descending 
                ? base.IClient.GetRangeWithScoresFromSortedSetDesc(key, start, stop)
                : base.IClient.GetRangeWithScoresFromSortedSet(key, start, stop);
        
            return results.ToDictionary(x => x.Key, x => x.Value);
        }

        // 异步版本
        public async Task<Dictionary<string, double>> GetRangeWithScoresAsync(string key, int start, int stop, bool descending = true)
        {
            return await Task.Run(() => GetRangeWithScores(key, start, stop, descending));
        }
        /// <summary>
        /// 获取key的所有集合
        /// </summary>
        ...
        /// <summary>
        /// 为key集合值为value的数据，分数减scoreby，返回相减后的分数/异步
        /// </summary>
        /// <param name="key"></param>
        /// <param name="value"></param>
        /// <param name="incrementBy"></param>
        /// <returns></returns>
        public async Task<double> IncrementItemInSortedSetAsync(string key, string value, double incrementBy)
        {
            return await Task.Run(() => base.IClient.IncrementItemInSortedSet(key, value, incrementBy));
        }
        /// <summary>
