...
namespace Medical.Infrastructure.Redis
{
    ...
    public class RedisListService:RedisBase
    {
       ...
       public async Task<long> LPushAsync(string key, string value)
        ...
        public async Task<List<string>> LRangeAsync(string key, long start, long stop)
        {
            return await Task.Run(() => 
                base.IClient.GetRangeFromList(key, (int)start, (int)stop));
        }

        public async Task<long> LTrimAsync(string key, long start, long stop)
        {
            return await Task.Run(() => 
            {
                base.IClient.TrimList(key, (int)start, (int)stop);
                return base.IClient.GetListCount(key);
            });
        }
        ...
        /// <returns></returns>
        public async Task<long> AddRangeAsync(string key, List<string> values)
        {
            // 将多个值添加到列表的尾部，并返回列表的长度
            return await Task.Run(() => 
            {
                base.IClient.AddRangeToList(key, values);
                return base.IClient.GetListCount(key);
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public async Task<long> AddAsync(string key, string value)
        {
            // 将值添加到列表的尾部，并返回列表的长度
            return await Task.Run(() => 
            {
        ...
        public async Task<List<string>> GetAllAsync(string key)
        {
            return await Task.Run(() => 
            {
                try
                {
                    return _client.GetAllItemsFromList(key);
                }
                catch
                {
        ...
        {
            return await Task.Run(() => base.IClient.GetListCount(key));
        }
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public async Task<long> RemoveAsync(string key, string value)
        {
            return await Task.Run(() =>
            {
                return base.IClient.RemoveItemFromList(key, value);
